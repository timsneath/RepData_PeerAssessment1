...
beginning at `r max.interval.asTime`.

# Imputing missing values
There are `r sum(is.na(activity$steps))` rows with missing values in the source
...
As a result of this, there are now `r sum(is.na(activity$steps))` rows with 
...
```
We find now that the mean number of steps taken per day is `r mean.perday2` 
and the median is `r median.perday2`.
