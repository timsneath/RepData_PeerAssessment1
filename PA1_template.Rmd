...
```
We find that the mean number of steps taken per day is `r mean.perday` and 
the median is `r median.perday`. To summarize:
```{r}
...
```
The interval with the maximum number of steps was the five minute period beginning at `r max.interval.asTime`.

## Imputing missing values
There are `r sum(is.na(activity$steps))` rows with missing values in the source data; however, we can impute them from the existing data. 
...
As a result of this, there are now `r sum(is.na(activity$steps))` rows with missing values. We can see how this affects the plot presented previously:
```{r}
...
```
We find now that the mean number of steps taken per day is `r mean.perday2` and the median is 
`r median.perday2`.
